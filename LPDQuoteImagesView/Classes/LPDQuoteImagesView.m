...
- (instancetype)initWithFrame:(CGRect)frame withCountPerRowInView:(NSUInteger)ArrangeCount cellMargin:(CGFloat)cellMargin{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteCellOfPhoto:) name:@"DeleteCellOfPhoto" object:nil];
...
- (void)configCollectionView {
    LPDPhotoArrangeCVlLayout *layout = [[LPDPhotoArrangeCVlLayout alloc] init];
...
- (void)deleteCellOfPhoto:(NSNotification *)info{
    
    NSInteger row = [info.userInfo[@"IndexPath"] integerValue];
    
    [_selectedPhotos removeObjectAtIndex:row];
    [_selectedAssets removeObjectAtIndex:row];
